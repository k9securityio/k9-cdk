...
Supported services: S3, KMS, DynamoDB, SQS.
...
### Service Modules: `src/s3.ts`, `src/kms.ts`, `src/dynamodb.ts`, `src/sqs.ts`
Each adapts `K9PolicyFactory` to its service's CDK API:
- **S3/SQS**: Use `grantAccessViaResourcePolicy()` — adds statements to existing resource policy via `addToResourcePolicy()`
...
- `test/k9.test.ts` — integration tests covering all services via the public API
...
- JSII compatibility required — this library is compiled with jsii for cross-language support. Avoid TypeScript features not supported by jsii (mapped types, conditional types, `Partial<T>`, etc.)
